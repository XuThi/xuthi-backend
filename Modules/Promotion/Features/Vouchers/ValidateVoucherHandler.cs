...
/// <summary>
/// Validates a voucher for cart/checkout. Returns discount amount if valid.
/// </summary>
    ...
    : IQueryHandler<ValidateVoucherQuery, ValidateVoucherResult>
{
    public async Task<ValidateVoucherResult> Handle(ValidateVoucherQuery query, CancellationToken ct)
    {
        ...
        // Check per-customer usage limit (future - requires Customer module)
        ...
        // Check first purchase only (future - requires Customer module)
        // if (voucher.FirstPurchaseOnly && query.CustomerId.HasValue) { ... }
        ...
        decimal discountAmount = voucher.Type switch
        {
            ...
            VoucherType.FreeShipping => voucher.DiscountValue, // Shipping cost as discount
            _ => 0
        };
